      ...
      <div className="container-shell py-16 lg:py-24">
        ...
        <div className="mt-12 grid gap-8 lg:grid-cols-3">
          {rangeCards.map((card, index) => (
            <article key={card.name} className="bg-warm">
              <p className="mb-2 text-[10px] font-bold uppercase tracking-[0.16em] text-forge/40">{card.tag}</p>
              <div className="h-64 border border-forge/20 bg-white p-3 shadow-soft">
              ...
              <p className="mt-3 min-h-16 text-sm leading-6 text-forge/65">{card.copy}</p>
              <div className="mt-5">
                {card.specs.map(([key, value]) => (
                  <div key={key} className="spec-line">
                    ...
                    <span className="max-w-[12rem] text-right font-bold text-forge/72">{value}</span>
                  </div>
                ))}
              </div>
              <a href="/products" className={`industrial-button mt-5 w-full ${index === 1 ? 'bg-charge' : 'bg-transparent'}`}>
                View tech specs
              </a>
            </article>
          ))}
